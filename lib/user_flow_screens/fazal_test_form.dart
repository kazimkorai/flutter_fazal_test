...
class FazalTest extends StatefulWidget {
  @override
  _RequestSupplierState createState() => _RequestSupplierState();
}

class _RequestSupplierState extends State<FazalTest> {
  ...
  Future fetchDropDown() async {
    final response = await http
        .get(ApiUrls.BASE_API_URL+'getdropdownapi');
  ...
  RequestSupplier() async
  {
    ...
    await http.post(myurl, headers: {
      ...
      else if(status=='Request created. But no supplier found in your mentioned address'){
        showDialog(
            context: context,
            builder: (BuildContext context) {
              return CustomDialog.con(
